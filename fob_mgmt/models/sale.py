...
class SaleOrder(models.Model):
    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            order.update({
                ...
                # 'total_pending_balance': amount_untaxed + amount_tax,
            })
    ...
    def _get_invoiced(self):
        ...
        line_invoice_status_all = [(d['order_id'][0], d['invoice_status']) for d in self.env['fob.order.line'].read_group([('order_id', 'in', self.ids), ('product_id', '!=', deposit_product_id.id)], ['order_id', 'invoice_status'], ['order_id', 'invoice_status'], lazy=False)]
        for order in self:
    ...
    order_line = fields.One2many('fob.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('fob.order'))
    ...
    def _compute_amount_undiscounted(self):
        ...
        for line in self.order_line:
            total += line.price_subtotal + line.price_unit * ((line.discount or 0.0) / 100.0) * (line.pairs_total - line.shortage_qty)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('fob.order') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('fob.order') or _('New')
        ...
        if any(f not in vals for f in ['partner_invoice_id', 'partner_shipping_id', 'pricelist_id']):
            ...
            #vals['currency_id'] = self.currency_selection_id
    ...
    def action_quotation_send(self):
        ...
        if template and template.lang:
            lang = template._render_template(template.lang, 'fob.order', self.ids[0])
        ctx = {
            'default_model': 'fob.order',
    ...
    def _amount_by_group(self):
        for order in self:
            ...
            for line in order.order_line:
                ...
                taxes = line.tax_id.compute_all(price_reduce, quantity=line.pairs_total, product=line.product_id, partner=order.partner_shipping_id)['taxes']
    ...
    #             line.qty_to_invoice = line.pairs_total - line.qty_invoiced
    #         else:
...
class SaleOrderLine(models.Model):
        ...
        for line in self:
            if line.state not in ('sale', 'done'):
                ...
                line.invoice_status = 'to invoice'
                    ...
                    float_compare(line.qty_delivered, line.pairs_total, precision_digits=precision) == 1:
                line.invoice_status = 'upselling'
            elif float_compare(line.qty_invoiced, line.pairs_total, precision_digits=precision) >= 0:
    ...
    def _get_to_invoice_qty(self):
        ...
        """
        #fob_journal_id = self.env['ir.config_parameter'].sudo().get_param('fob_mgmt.fob_journal_id')
        fob_tax_id = self.env['ir.config_parameter'].sudo().get_param('fob_mgmt.fob_tax_id')
        try:
            default_tax_id = int(fob_tax_id)
        except:
            default_tax_id = 87
        for line in self:
            line.tax_id = [(6, 0, [default_tax_id])] #[(6, 0, [27])]
    ...
    def _fob_custom(self):
        po_line_obj = self.env['fob.po.order.line']
        po_line_ids = po_line_obj.search([('pi_id', '=', self._origin.id)])
        if po_line_ids:
            po_line = po_line_obj.browse(po_line_ids[0].id)
            if self.product_uom_qty > po_line.product_qty:
                return {'warning': {'title': 'warning',
                                'message': 'La riga è associata ad una quantità inferiore (%s) nel PO %s' % (
                                str(po_line.product_qty), str(po_line.order_id.name)), 'sticky': False}}
            elif self.product_uom_qty < po_line.product_qty:
                return {'warning': {'title': 'warning', 'message': 'La riga è associata ad una quantità maggiore (%s) nel PO %s' % (str(po_line.product_qty),str(po_line.order_id.name)), 'sticky': True}}
    ...
    def _get_price_reduce_tax(self):
        for line in self:
            line.price_reduce_taxinc = line.price_total / line.pairs_total if line.pairs_total else 0.0

    @api.depends('price_subtotal', 'product_uom_qty')
    def _get_price_reduce_notax(self):
        for line in self:
            line.price_reduce_taxexcl = line.price_subtotal / line.pairs_total if line.pairs_total else 0.0
    ...
    _sql_constraints = [
        ...
        ('non_accountable_null_fields',
            "CHECK(display_type IS NULL OR (price_unit = 0 AND product_uom_qty = 0 AND product_uom IS NULL AND customer_lead = 0))",
    ...
    #             line.pairs_total, float(values['pairs_total']),)
    ...
    def write(self, values):
        ...
        if 'pairs_total' in values:
            precision = self.env['decimal.precision'].precision_get('Product Unit of Measure')
            self.filtered(lambda r: r.state == 'sale' and float_compare(r.pairs_total, values['pairs_total'], precision_digits=precision) != 0)._update_line_quantity(values)
    ...
    order_id = fields.Many2one('fob.order', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    ...
    product_uom = fields.Many2one('uom.uom', string='Unit of Measure', default=21)
    ...
    def _get_display_price(self, product):
        # TO DO: move me in master/saas-16 on fob.order
        ...
        final_price, rule_id = self.order_id.pricelist_id.with_context(product_context).get_product_price_rule(self.product_id, self.pairs_total or 1.0, self.order_id.partner_id)
        base_price, currency = self.with_context(product_context)._get_real_price_currency(product, rule_id, self.pairs_total, self.product_uom, self.order_id.pricelist_id.id)
    ...
    def product_id_change(self):
        ...
        if not self.product_uom or (self.product_id.uom_id.id != self.product_uom.id):
            ...
            vals['pairs_total'] = self.pairs_total or 1.0

        product = self.product_id.with_context(
            lang=self.order_id.partner_id.lang,
            partner=self.order_id.partner_id,
            quantity=vals.get('pairs_total') or self.pairs_total,
