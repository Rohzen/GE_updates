...
class WizardExportFatturapa(models.TransientModel):
    ...
    def setDatiGeneraliDocumento(self, invoice, body):
        ...
        if invoice.currency_id.name == 'USD':
            try:
                if invoice.total_value_price:
                    ImportoTotaleDocumento = invoice.total_value_price
                else:
                    ImportoTotaleDocumento = invoice.amount_total
            except:
                ImportoTotaleDocumento = invoice.amount_total
        elif invoice.currency_id.name == 'EUR':
                ImportoTotaleDocumento = invoice.amount_total
    ...
    def _get_prezzo_unitario_usd(self, line):
        res = line.value_unit_price
        return res
    ...
    def setDettaglioLinea(
        ...
        price_subtotal = 0
        if line.invoice_id.currency_id.name == 'USD':
            prezzo_unitario = self._get_prezzo_unitario_usd(line)
            price_subtotal = line.value_price
        elif line.invoice_id.currency_id.name == 'EUR':
        ...
        DettaglioLinea = DettaglioLineeType(
            ...
            PrezzoTotale='%.2f' % float_round(price_subtotal, 2),
        ...
        for tax_line in invoice.tax_line_ids:
            ...
            if invoice.currency_id.name == 'USD':
                riepilogo = DatiRiepilogoType(
                    ...
                    ImponibileImporto='%.2f' % float_round(invoice.total_value_price, 2),
